...
async def main(bot: bot_core.GuardBot, *, member: discord.Member):
    ...
    template: dict = bot.db.execute("FROM Templates SELECT data IF (ServerID == ?)", server_id)
    role_ids = bot.db.execute("FROM Roles SELECT RoleID IF (ServerID == ?)", server_id)
