...
1) изменение скрипта [on_voice_state_update](scripts/on_voice_state_update.py) - добавление кулдауна
2) изменение в пре-процессинге include
3) попытка исправления (`/clear_queue`) + рефакторинг [VoiceCog](bot/cogs/voice.py)
4) добавление

### `фиксы`

- исправления некорректной загрузки скриптов (импорт скриптов из скриптов мог вызывать ошибку на Linux)
- `/clear_queue` - попытки изменения
